...
void L1Controller::goalCB(const geometry_msgs::msg::PoseStamped::SharedPtr goalMsg)
{
  ...
  msg = *goalMsg;
  // Fix: Rviz2 sometimes publishes goal_pose with empty frame_id, default to "map"
  if (msg.header.frame_id.empty()) {
    msg.header.frame_id = "map";
  }
  goal_pos = msg;
  current_time = rclcpp::Clock().now();
  RCLCPP_INFO(this->get_logger(), "Goal received, frame_id: %s", msg.header.frame_id.c_str());
  try
  {
    tf_buffer_->transform(msg, odom_goal, "odom", tf2::durationFromSec(1.0));
